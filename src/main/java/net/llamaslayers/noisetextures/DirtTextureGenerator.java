...
public class DirtTextureGenerator extends AbstractTextureGenerator {
	private static final double ESPILON = 0.00001;
	private static final double[] ANGLE_MODIFIERS = {2, 1.5, 1, 0.5, 1, 1.5, 2, 2.5};
	private final SimplexOctaveGenerator noisy;
	private final SimplexOctaveGenerator noise;

	public DirtTextureGenerator(long seed) {
		super(seed);
		Random r = getSeed();
		noisy = getNoise(r, 4);
		noisy.setScale(1 / 64.0);
		noise = getNoise(r, 4);
	...
	public byte getTone(double x, double y) {
		return (byte) Math.min(Math.max(noise.noise(x, y, 0.5, 0.5, true) * 40
						* lerp(ANGLE_MODIFIERS[getUp(x, y)], 0.1, noisy.noise(x, y, 0.5, 0.5, true))
						+ 120, 0), 255);
	}
		...
		for (double _x = x; _x < x + width; _x += 1.0 / step) {
			for (double _y = y; _y < y + height; _y += 1.0 / step) {
				b[i] = (byte) Math.min(Math.max(noise.noise(_x, _y, 0.5, 0.5, true) * 40
						* lerp(ANGLE_MODIFIERS[getUp(_x, _y)], 0.1, noisy.noise(_x, _y, 0.5, 0.5, true))
						+ 120, 0), 255);
				i++;
			}
		}
	...
	private double lerp(double max, double min, double normal) {
		if (normal <= 0)
			return min;
		return max * normal + min * (1 - normal);
	}
}
